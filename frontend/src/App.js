...
const CartProvider = ({ children }) => {
  const [cart, setCart] = useState({ items: [], total: 0, subtotal: 0, gst: 0 });
  ...
  const updateCartState = useCallback(async (items) => {
    localStorage.setItem("gothra_cart_items", JSON.stringify(items));
    if (items.length === 0) {
      setCart({ items: [], total: 0, subtotal: 0, gst: 0 });
      return;
    }
    try {
      const response = await axios.post(`${API}/cart/calculate`, { items });
  ...
  const fetchCart = useCallback(async () => {
    const stored = localStorage.getItem("gothra_cart_items");
    let items = [];
    if (stored) {
      try {
  ...
  const addToCart = async (productId, quantity = 1) => {
    ...
    try {
      ...
      let currentItems = [];
      if (stored) {
        try { currentItems = JSON.parse(stored); } catch (e) {}
      }

      const itemIdx = currentItems.findIndex(i => i.product_id === productId);
      if (itemIdx !== -1) {
        currentItems[itemIdx].quantity += quantity;
      } else {
        currentItems.push({ product_id: productId, quantity });
      }

      await updateCartState(currentItems);
  ...
  const updateCartItem = async (productId, quantity) => {
    ...
    try {
      ...
      if (quantity <= 0) {
        currentItems = currentItems.filter(i => i.product_id !== productId);
      } else {
        const itemIdx = currentItems.findIndex(i => i.product_id === productId);
        if (itemIdx !== -1) {
          currentItems[itemIdx].quantity = quantity;
  ...
  const clearCart = async () => {
    localStorage.removeItem("gothra_cart_items");
    setCart({ items: [], total: 0, subtotal: 0, gst: 0 });
  };
            ...
            <>
              <div className="flex-1 overflow-auto space-y-2 pr-1 min-h-0">
                  ...
                  <div key={item.product_id} className="flex gap-3 p-2.5 bg-white rounded-lg" data-testid={`cart-item-${item.product_id}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <button onClick={() => updateCartItem(item.product_id, item.quantity - 1)} className="p-2 md:p-1 hover:bg-[#F3EBE1] rounded flex items-center justify-center min-w-[32px] min-h-[32px] md:min-w-0 md:min-h-0" data-testid={`decrease-${item.product_id}`}><Minus size={16} className="md:w-3.5 md:h-3.5" /></button>
                        <span className="text-sm font-medium w-6 text-center">{item.quantity}</span>
                        <button onClick={() => updateCartItem(item.product_id, item.quantity + 1)} className="p-2 md:p-1 hover:bg-[#F3EBE1] rounded flex items-center justify-center min-w-[32px] min-h-[32px] md:min-w-0 md:min-h-0" data-testid={`increase-${item.product_id}`}><Plus size={16} className="md:w-3.5 md:h-3.5" /></button>
                        <button onClick={() => updateCartItem(item.product_id, 0)} className="ml-auto p-2 md:p-1 text-[#C05A42] hover:bg-red-50 rounded flex items-center justify-center min-w-[32px] min-h-[32px] md:min-w-0 md:min-h-0" data-testid={`remove-${item.product_id}`}><Trash2 size={16} className="md:w-3.5 md:h-3.5" /></button>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <div className="pt-2 space-y-1.5">
                  ...
                  <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Full name *" className="w-full px-3.5 py-2.5 md:py-2 rounded-lg bg-white border border-[#EAD8C3] text-base md:text-sm text-[#1A2421] placeholder:text-[#4A5D54]/50 outline-none focus:border-[#1E3F33]" data-testid="checkout-name-input" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                    <input type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder="Email *" className="w-full px-3.5 py-2.5 md:py-2 rounded-lg bg-white border border-[#EAD8C3] text-base md:text-sm text-[#1A2421] placeholder:text-[#4A5D54]/50 outline-none focus:border-[#1E3F33]" data-testid="checkout-email-input" />
                    <input type="tel" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="Phone *" className="w-full px-3.5 py-2.5 md:py-2 rounded-lg bg-white border border-[#EAD8C3] text-base md:text-sm text-[#1A2421] placeholder:text-[#4A5D54]/50 outline-none focus:border-[#1E3F33]" data-testid="checkout-phone-input" />
                  </div>
                  <input type="text" value={addressLine} onChange={(e) => setAddressLine(e.target.value)} placeholder="Address (House/Street) *" className="w-full px-3.5 py-2.5 md:py-2 rounded-lg bg-white border border-[#EAD8C3] text-base md:text-sm text-[#1A2421] placeholder:text-[#4A5D54]/50 outline-none focus:border-[#1E3F33]" data-testid="checkout-address-input" />
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-1.5">
                    <input type="text" value={city} onChange={(e) => setCity(e.target.value)} placeholder="City *" className="w-full px-3.5 py-2.5 md:py-2 rounded-lg bg-white border border-[#EAD8C3] text-base md:text-sm text-[#1A2421] placeholder:text-[#4A5D54]/50 outline-none focus:border-[#1E3F33]" data-testid="checkout-city-input" />
                    <input type="text" value={state} onChange={(e) => setState(e.target.value)} placeholder="State *" className="w-full px-3.5 py-2.5 md:py-2 rounded-lg bg-white border border-[#EAD8C3] text-base md:text-sm text-[#1A2421] placeholder:text-[#4A5D54]/50 outline-none focus:border-[#1E3F33]" data-testid="checkout-state-input" />
                    <input type="text" value={pincode} onChange={(e) => setPincode(e.target.value)} placeholder="PIN *" className="w-full px-3.5 py-2.5 md:py-2 rounded-lg bg-white border border-[#EAD8C3] text-base md:text-sm text-[#1A2421] placeholder:text-[#4A5D54]/50 outline-none focus:border-[#1E3F33]" data-testid="checkout-pincode-input" />
                  </div>
                </div>
              </div>
              ...
              <div className="flex-shrink-0 border-t border-[#EAD8C3] pt-2 pb-[env(safe-area-inset-bottom,8px)] bg-[#FAF8F5]">
                ...
                <Button onClick={handleCheckout} disabled={isLoading} className="w-full bg-[#1E3F33] hover:bg-[#152D24] rounded-full h-14 md:h-12 text-base mb-6" data-testid="checkout-btn">
...
const HeroSection = () => (
  <section className="pt-20 relative" data-testid="hero-section">
    <div className="w-full">
      <img src="/images/vu81syzr_gothra.jpeg" alt="GOTHRA" className="w-full h-auto object-contain" />
    </div>
  </section>
);
  ...
  <section className="py-10 md:py-16 bg-[#FAF8F5]" data-testid="home-about-section">
    <div className="w-full px-2 md:px-8">
      <div className="flex justify-center">
        <img 
          src="/images/di2od5vi_Screenshot 2026-04-14 184653.png" 
          alt="GOTHRA About Us" 
...
const HomeProductsSection = () => {
  ...
  const SmallProductCard = ({ product }) => (
    <div className="bg-white p-2 md:p-3 text-center group" data-testid={`product-${product.id}`}>
      ...
      <Button onClick={() => addToCart(product.id)} size="sm" className="mt-2.5 bg-[#1E3F33] hover:bg-[#152D24] rounded-full text-sm w-full h-10 md:h-9">Add</Button>
    </div>
  );
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
            {juteCurtains.map((product) => (
              <div key={product.id} className="bg-white p-3 md:p-4 group">
                ...
                <Button onClick={() => addToCart(product.id)} size="sm" className="mt-2.5 w-full bg-[#1E3F33] hover:bg-[#152D24] rounded-full text-sm h-10 md:h-9">Add to Cart</Button>
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={product.id} className="bg-white rounded-lg overflow-hidden group hover:shadow-lg transition-shadow" data-testid={`shop-product-${product.id}`}>
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <Button onClick={() => addToCart(product.id)} size="sm" className="bg-[#1E3F33] hover:bg-[#152D24] rounded-full text-sm px-5 h-10 md:h-9">Add</Button>
                  </div>
                </div>
              </div>
            ))}
  ...
  <div className="pt-20" data-testid="about-page">
    <section className="relative h-[40vh] flex items-center justify-center">
      <div className="absolute inset-0 z-0">
        <img src="/images/vu81syzr_gothra.jpeg" alt="GOTHRA" className="w-full h-full object-cover" />
    ...
    <section className="py-16 bg-[#FAF8F5]">
      <div className="w-full px-4 md:px-8">
        <div className="flex justify-center">
          <img src="/images/di2od5vi_Screenshot 2026-04-14 184653.png" alt="GOTHRA About Us" className="w-full max-w-[1400px] h-auto object-contain" data-testid="about-page-store-image" />
        </div>
      </div>
    </section>
